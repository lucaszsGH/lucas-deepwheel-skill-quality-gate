...
状态：当前版本 1.0.0；只有当前提交通过全部校验门禁，才可视为可安装。
...
- 结构与 frontmatter；
- 能力声明和相邻任务边界；
- 命名规则（对照可配置的名称前缀 `SKILL_QUALITY_GATE_NAME_PREFIX`）；
- 独立产品入口与关联 Skill 路由；
- 新用户能力体检；
- Token 与交互策略；
- 常见凭证形状、本机路径、个人信息和原始残留；
- GitHub 发布准备；
- 修改后只读对账本地源码、GitHub main/PR、Actions、VERSION/Tag/Release 和本机安装版；
- README、中英文可编辑/渲染介绍图片、示例与安装说明是否仍与当前能力一致；
- 机器门禁是否能用退出码真正阻断 CI。
...
不修改任何状态，只读核验仓库、GitHub、Actions、发布元数据和安装版：
...
- Skill 本体和发布包的静态审计；
- 修改后的 GitHub 与安装版只读对账；
- 用能力指纹绑定 README、双语 SVG/PNG 介绍、示例、安装说明和 Changelog；
- 公开介绍未随能力复核时输出 `VISUAL ASSET STALE`；
- 安全的 JSON 或人类可读输出；
- 可阻断 CI 的退出码；
...
- 检出所有凭证或个人信息；
- 证明全部业务能力可用；
- 自动修复、生成 README/图片、安装、发布、push、Tag 或 Release；
- 替代正式安全审查。
...
每个高风险发布包还必须声明可执行的行为安全合同和发布包测试文件，覆盖同意、数据主体、最小输入、安全预检、停止路由、阻断输出和来源追溯。门禁只静态核验这些证据，不执行不受信任的目标代码；实际测试由目标仓库 CI 执行。
